...
namespace Tac
{
	...
	public class EntityManager 
	{
		...
		/// <summary>
		/// Get all entities that match all of the given attribute queries.
		/// Basically a big, dumb, expensive database.
		/// </summary>
		public List<Entity> GetEntities (params AttrQuery[] queries)
		{
			List<Entity> results = new List<Entity>();
			foreach (Entity entity in _entities)
			{
				if (entity.Matches(queries))
				{
					results.Add(entity);
				}
			}
		...
		public Entity SpawnGeneric (Entity owner)
		{
		...
		public Entity SpawnByDefinition (string definition, Entity owner)
		{
			Entity entity = new Entity(definition, owner);
			_entities.Add(entity);
			return entity;
		}

		// public T SpawnByDefinition<T> (string definition, Entity owner) where T : Entity, new()
		// {
		// 	T entity = (T)System.Activator.CreateInstance(typeof(T), definition, owner);
		// 	_entities.Add(entity);
		// 	return entity;
		// }

		public static Entity SpawnById (string id, Entity creator)
		{
			// See if this id exists in our database
